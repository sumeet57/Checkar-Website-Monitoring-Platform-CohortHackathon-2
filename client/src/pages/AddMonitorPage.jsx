...
const TextField = ({ label, error, children }) => (
	<div>
		<label className="mb-2 block font-mono text-xs uppercase tracking-widest text-zinc-500">{label}</label>
		{children}
	...
	const onSubmit = async (values) => {
		try {
			...
			if (isEditMode) {
				...
				dispatch(updateJobThunk.fulfilled(updatedJob, '', { id: editId, data: payload }));
		...
		<div className="flex flex-wrap gap-2">
			{tabOptions.map((tab) => (
				<button
					...
					className={`rounded-full border px-4 py-2 font-mono text-xs uppercase tracking-widest transition ${activeTab === tab.key ? "border-orange-400 bg-orange-400 text-zinc-950" : "border-zinc-700 bg-transparent text-zinc-300 hover:bg-zinc-800 hover:text-zinc-50"}`}
				>
					{tab.label}
				</button>
			))}
		...
		<>
			<TextField label="Title" error={fieldError(errors, "title")}>
				<input
					...
					className="w-full rounded border border-[#2a2a3d] bg-[#0d0d14] px-4 py-3 font-mono text-sm text-white placeholder-[#374151] focus:border-[#7c3aed] focus:outline-none focus:ring-0"
				/>
			</TextField>

			<div>
				<label className="mb-2 block font-mono text-xs uppercase tracking-widest text-zinc-500">Interval</label>
				{activeTab === "ssl" ? (
					<div className="rounded-lg border border-zinc-700 bg-zinc-950 px-4 py-3 font-mono text-sm text-zinc-300">86400 seconds (daily)</div>
				) : (
					<div className="grid grid-cols-4 gap-2">
						{(activeTab === "frontend" ? frontendIntervals : intervalOptions).map((value) => (
							<button
								...
								className={`rounded border px-3 py-3 font-mono text-sm transition ${Number(intervalValue) === value ? "border-[#7c3aed] bg-[#7c3aed] text-white" : "border-[#2a2a3d] bg-transparent text-[#6b7280] hover:text-white"}`}
							>
								{value}
							</button>
						))}
		...
		<TextField label="URL" error={fieldError(errors, "url")}>
			<div className="relative">
				<input
					...
					className="w-full rounded border border-[#2a2a3d] bg-[#0d0d14] px-4 py-3 pr-10 font-mono text-sm text-white placeholder-[#374151] focus:border-[#7c3aed] focus:outline-none focus:ring-0"
				/>
				{isValidHttps ? <span className="absolute right-3 top-1/2 -translate-y-1/2 font-mono text-sm text-emerald-400">✓</span> : null}
			</div>
		</TextField>
	);
								...
								<div className="flex flex-wrap gap-2">
									{commonPorts.map((port) => (
										<button
											...
											className="rounded border border-[#2a2a3d] px-3 py-2 font-mono text-xs text-[#9ca3af] hover:text-white"
										>
											{port}
										</button>
									))}
			...
			case "frontend":
				return (
					<div className="space-y-5">
						{renderUrlField()}
						<div>
							<label className="mb-2 block font-mono text-xs uppercase tracking-widest text-[#9ca3af]">Interval</label>
						...
						<TextField label="Timeout (ms)" error={fieldError(errors, "expectations.timeout")}>
							<select
								...
								className="w-full rounded border border-[#2a2a3d] bg-[#0d0d14] px-4 py-3 font-mono text-sm text-white focus:border-[#7c3aed] focus:outline-none focus:ring-0"
							>
						...
						<div className="flex items-center justify-between rounded border border-[#2a2a3d] bg-[#0d0d14] px-4 py-3">
							<div>
								<div className="font-mono text-xs uppercase tracking-widest text-[#9ca3af]">Check console</div>
								<div className="mt-1 font-mono text-xs text-[#6b7280]">Capture browser console errors while checking the page.</div>
							</div>
							<input {...register("expectations.checkConsole")} type="checkbox" className="h-4 w-4 accent-[#7c3aed]" />
						</div>

						<div className="flex items-center justify-between rounded border border-[#2a2a3d] bg-[#0d0d14] px-4 py-3 opacity-80">
							<div>
								<div className="font-mono text-xs uppercase tracking-widest text-[#9ca3af]">Headless</div>
								<div className="mt-1 font-mono text-xs text-[#6b7280]">Always enabled for frontend checks.</div>
							</div>
							<div className="rounded border border-[#2a2a3d] px-3 py-2 font-mono text-xs text-white">true</div>
					...
					<div className="mx-auto max-w-2xl">
						<div className="mb-6 flex items-center gap-4">
							<button
								type="button"
								onClick={() => navigate(-1)}
								className="rounded-full border border-zinc-700 px-3 py-2 font-mono text-xs uppercase tracking-widest text-zinc-300 hover:bg-zinc-800 hover:text-zinc-50"
							>
								← Back
							</button>
							<div>
								<h1 className="text-3xl font-semibold text-zinc-50">{isEditMode ? "Edit monitor" : "Create monitor"}</h1>
							</div>
						</div>
								...
								<div className="flex gap-3">
									{isEditMode && (
										<button
											type="button"
											onClick={() => navigate(-1)}
											className="flex-1 rounded-full border border-zinc-700 px-4 py-3 font-mono text-sm text-zinc-300 transition hover:bg-zinc-800"
										>
											Cancel
										</button>
									)}
									<button
										...
										className="flex-1 rounded-full bg-orange-400 px-4 py-3 font-mono text-sm text-zinc-950 shadow-glow-orange transition hover:bg-orange-500 disabled:cursor-not-allowed disabled:opacity-60"
									>
										{isSubmitting
